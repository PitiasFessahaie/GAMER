            ...
            if ch[0] >= 0:
                if ch[0] < self.size[1] - 1:
                    if ch[1] >= 0:
                        if ch[1] < self.size[0]:
                            try:
                                self.win.addstr(int(ch[1]), int(ch[0]), chr(
                                    ch[2]), curses.color_pair(ch[3]))
                            except:
                                pass
        if debug != None:
    ...
    def render(self, tick, screenspace):
        ...
        for i in range(self.length):
            if i == 0:
                ...
                self.pendingcolor = self.colors[int(
                    (i / self.length) * self.lcolors)]
    ...
    try:
        while True:
            ...
            if PARAMS.debug == True:
                ...
                deb.append("%s\tframes, delay %s, colors %s, fades %s" % (
                    str(TICK), str(PARAMS.delay), PARAMS.colors, str(PARAMS.fades)))
            SCSP.render(deb)
            TICK += 1
...
if __name__ == '__main__':
    agp = argparse.ArgumentParser(
        description="A cool matrix effect for your terminal!")
